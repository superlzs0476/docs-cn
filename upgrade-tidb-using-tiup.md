...
- 3.0 之前的版本，需要先通过 TiDB Ansible 升级到 3.0 版本，然后按照本文档的说明，使用 TiUP 将 TiDB Ansible 配置导入，再升级到 4.0.0 及之后版本。
...
- 从 2.0.6 之前的版本升级到 4.0.0 及之后版本之前，需要确认集群中是否存在正在运行中的 DDL 操作，特别是耗时的 `Add Index` 操作，等 DDL 操作完成后再执行升级操作
- 2.1 及之后版本启用了并行 DDL，早于 2.0.1 版本的集群，无法滚动升级到 4.0.0 及之后版本，可以选择下面两种方案：
    - 停机升级，直接从早于 2.0.1 的 TiDB 版本升级到 4.0.0 及之后版本
    - 先滚动升级到 2.0.1 或者之后的 2.0.x 版本，再滚动升级到 4.0.0 及之后版本
